...
if(isset($_POST["post_title"]) && isset($_POST["post_content"]))
  {
    ...
    //*************************USER*****************************
    $user = $_SESSION["user"];
    $sql= $mySqli_db->prepare("INSERT INTO `blog_posts`(`title`, `content`, `user`, `blog_id`) VALUES (?, ?, ?, ?)");
    $sql->bind_param("sssi",$post_title, $post_content, $user, $_GET["blog"]);
    $sql->execute();
...
if(isset($_GET["blog"]))
  {
    ...
    {
      echo '<form action="index.php?page=blog&blog=' . $_GET["blog"] . '" method="post" role="form">
              <div class="row">
    ...
    for($i=0; $i<$result->num_rows; $i++)
    {
     ...
     /* $sql= $mySqli_db->prepare("SELECT * FROM forum_categories fc INNER JOIN forum_categories_relation fcr ON fc.id = fcr.forum_category_id WHERE fcr.blog_id = ?");
      ...
      echo '<h2 class="mb-3"><strong>
            <a class="text-dark" href="index.php?page=blog&blog=' . $row["id"] . '">' . str_replace("\'", "'",str_replace("\\\"", "\"", $row["title"])) . '</a>
          </strong></h2>
        ...
        echo '</a>
          ...
          <a href="index.php?page=blog&blog=' . $row["id"] . '">Continue reading</a>
        </div>
